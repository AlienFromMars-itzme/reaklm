    ...
    if (!user) {
      ...
      if (subcmd === `add`) {
        if (np) {
            ...
            return message.channel.send({embeds: [ new client.embed().desc(`**Added** \`${user.tag}\` to my list\n${dur ? `**Ends**: <t:${Math.round((Date.now() + dur) / 1000)}:f>` : ""}`).setAuthor({name: `Manage NoPrefix`, iconURL: client.user.displayAvatarURL()})]})
          }
      ...
      return message.channel.send({embeds: [ new client.embed().desc(`**Removed** \`${user.tag}\` from my list`).setAuthor({name: `Manage NoPrefix`, iconURL: client.user.displayAvatarURL()})]})
        }
      }
